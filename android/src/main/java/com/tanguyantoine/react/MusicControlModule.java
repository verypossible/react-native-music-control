...
public class MusicControlModule extends ReactContextBaseJavaModule implements ComponentCallbacks2 {
    ...
    private void updateNotificationMediaStyle() {
        if (!Build.MANUFACTURER.toLowerCase(Locale.getDefault()).contains("huawei") && !Build.MANUFACTURER.toLowerCase(Locale.getDefault()).contains("amazon") && Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
